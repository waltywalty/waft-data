"""Round 49 attempt 30: VIX-shock reversal (frozen per
reference/goal_ledger.md, adjacency diagnosis on record). 4 selectable
cells + vol-crush diagnostics, single OOS evaluation.
Outputs results/r49b_vixshock.json."""
...
print(f"VIX: {len(vix)} days; shock days z>=1.5: {(z >= 1.5).sum()}, z>=2: {(z >= 2).sum()}, "
      f"crush z<=-1.5: {(z <= -1.5).sum()}")
...
print("\n=== IS grid (indices pooled, ATR-normalized; crush cells diagnostic) ===")
print(f"{'kind':>11} {'thr':>5} {'hold':>4} | {'n':>5} {'WR':>6} {'PF':>5} {'avgR':>7} {'t':>6} {'halves':>12} | {'gold avgR':>9}")
for r in rows:
    a, gg = r["IS"], r["IS_gold"]
    if a.get("n", 0) < 10: continue
    print(f"{r['kind']:>11} {r['thr']:>5} {r['hold']:>4} | {a['n']:>5} {a['wr']*100:>5.1f}% {a['pf']:>5.2f} "
...
if winner is not None:
    cr = [r for r in rows if r["kind"] == "crush_diag" and r["hold"] == winner["hold"]]
    if cr and (cr[0]["IS"].get("avg_R") or -9) >= (winner["IS"].get("avg_R") or -9):
        print(f"\nShock cell passes floor but crush diagnostic matches it "
              f"({cr[0]['IS'].get('avg_R'):+.3f} vs {winner['IS'].get('avg_R'):+.3f}): "
              "drift, family self-refutes at IS; OOS not opened.")
        json.dump(dict(split=split, grid=[{x: v for x, v in r.items() if not x.startswith("_")} for r in rows],
                  ...
                  open("results/r49b_vixshock.json", "w"), indent=1, default=float)
        raise SystemExit

if winner is None:
    print("\nNo shock cell passes the IS floor; family fails at IS, OOS not opened.")
    json.dump(dict(split=split, grid=[{x: v for x, v in r.items() if not x.startswith("_")} for r in rows],
