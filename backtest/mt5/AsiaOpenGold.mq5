...
input string          InpAudSymbol      = "AUDUSD";   // AUDUSD symbol (match your broker's suffix)
input group           "Signal"
input int             InpRangeMinutes   = 60;         // Opening range length, minutes
input int             InpRangeStartUtcH = 1;          // Range start hour, UTC
input int             InpRangeStartUtcM = 30;         // Range start minute, UTC
input int             InpEntryCutoffLdn = 8;          // No entry after this hour, London time
input group           "Correlation filter"
input bool            InpUseFilter      = true;       // Apply the correlation filter
input int             InpCorrDays       = 20;         // Rolling window, days
input double          InpCorrMax        = 0.50;       // Trade only when correlation <= this
input group           "Risk"
input double          InpRiskPercent    = 1.0;        // Risk per trade, % of equity
input double          InpStopRangeMult  = 2.0;        // Stop distance, multiples of range width
input double          InpMaxLots        = 0.0;        // Hard lot cap (0 = no cap)
input group           "Execution"
input int             InpMagic          = 930115;     // Magic number
