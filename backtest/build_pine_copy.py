...
{card("fp2", "FP2 RVOL Engine", "Participation anomalies: volume vs its own time-of-day baseline (EMA per minute bucket) plus session-cumulative RVOL. The one place execution algos betray themselves. Real CME volume required.", fp[2])}
...
{card("fp4", "FP4 Absorption Proxy", "Effort-vs-result: high volume percentile + low range percentile at an N-bar extreme, with close-location direction. Includes a lower-timeframe CVD PROXY, labeled as such - Pine has no true aggressor delta.", fp[4])}
{card("fp5", "FP5 Displacement Meter", "Initiative vs rotational regime from TR/ATR with body dominance, plus an on-chart follow-through scoreboard that prints this symbol's actual next-bar base rate (usually a coin flip - that is the lesson).", fp[5])}
