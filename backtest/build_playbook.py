...
<div class="wrap">
<header>
<p class="eyebrow">Living document &middot; last updated 2026-08-27 &middot; after round 28</p>
...
<li><strong>Portfolio reference split (adopted after round 28e):</strong> the
four-stream paper book runs at <strong>gold 35% / Double Seven 40% / HSI fade
25%</strong> &mdash; the max-MAR corner of a broad plateau (any balanced split
inside gold 15&ndash;40 / D7 40&ndash;70 / MHI 0&ndash;25 is statistically the
same portfolio, so the round numbers stand and are never re-optimized on a
rolling window). Sleeve correlations are ~0.00. In-sample reference:
~10.6%/yr at 5.1% max drawdown unlevered; leverage only enters after a sleeve
is SPRT-promoted and its live record confirms the backtest envelope.</li>
...
risk.</li>
<li>Rounds 25-28 sweeps: intraday-computed correlation gates (max-stat p 0.95)
and every entry timeframe below 30m (cost-dominated, monotone gradient);
the SGE auction candle (real at max-stat p 0.004, unmonetizable: 50.8% hit
rate, t +1.22 at ZERO cost); FVG/iFVG on four markets (0 of 12 cells, several
significantly negative); gold/dollar as lag-1 signals for ES/NQ/RTY (36
cells, max-stat p 0.86); and the researched-recipe battery &mdash; Hikkake,
Holy Grail, TTM Squeeze, Momentum Pinball all dead at frozen published
parameters, the Market Profile &ldquo;80% rule&rdquo; measured at 46%/32%
actual fill, Crabel&rsquo;s 60-76% win rates measured at 26-30%, and the
Zarattini noise-area strategy real gross but dead at our costs. One survivor
crossed to the watch list: Double Seven on SPX (item 4).</li>
</ul>
...
numbers only.
(4) Double Seven on SPX (round 28, Connors/Alvarez replication): close above
the 200-day SMA, buy a 7-day closing low, exit a 7-day closing high, no stop,
long only &mdash; PF 2.30, win 80%, halves 2.40/2.28 on 20 years, and the
2015-2025 half is fully post-publication out-of-sample; beats a matched-hold
drift null at p 0.005 but not the round&rsquo;s strict Bonferroni, so it
waits here at frozen parameters. Equity-index-specific (gold and RTY sit
inside the drift null); ~13 signals/yr; the published form carries no stop,
so size accordingly if it is ever papered.</p></div>
