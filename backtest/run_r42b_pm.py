"""Round 42 attempt 2: late-day intraday momentum (Gao-Han-Li-Zhou analogue).
Frozen per reference/goal_ledger.md. 12-variant IS grid, pre-stated selection,
single OOS evaluation. Outputs results/r42b_pm.json."""
...
def build_days(idx):
    ...
    for skey, g in rth.groupby("skey"):
        ...
        rows.append(dict(skey=skey, o930=o[0], c1000=last_before(1000),
                         c1500=last_before(1500), c1530=last_before(1530),
...
PREDS = {"first30": lambda d: d.p_first,
         "day": lambda d: d.p_day,
...
def neighbors(v):
    return [r for r in rows if sum(r[k] == v[k] for k in ("pred", "entry", "filt")) == 2]
...
for cand in ranked:
    ...
    if len(nb) == 0 or pos >= len(nb) / 2:
        winner = cand
        winner["_npos"] = f"{pos}/{len(nb)}"
        break

print("=== IS ranking (pooled, ATR-normalized) ===")
print(f"{'pred':>8} {'entry':>6} {'filt':>8} | {'n':>5} {'WR':>6} {'PF':>5} {'avgR':>7} {'t':>6} {'halves':>12}")
for r in ranked:
    a = r["IS"]
    print(f"{r['pred']:>8} {r['entry']:>6} {r['filt']:>8} | {a['n']:>5} {a['wr']*100:>5.1f}% "
...
if winner is None:
    ...
    json.dump(dict(split=split,
              ...
              open("results/r42b_pm.json", "w"), indent=1, default=float)
    raise SystemExit

print(f"\nSELECTED: {winner['pred']} {winner['entry']} {winner['filt']} (neighbors positive {winner['_npos']})")
sub = winner["_sub"]
oos = sub[sub.oos]
...
c15 = stats(oos.pnl - 0.5 * oos.pnl.index.map(lambda i: 0).astype(float)
            if False else (oos.pnl - 0.5 * pd.Series({k: MICRO[v] for k, v in oos.idx.items()})), oos.atr)
...
json.dump(dict(split=split,
               grid=[{k: v for k, v in r.items() if k not in ("_sub", "_npos")} for r in rows],
