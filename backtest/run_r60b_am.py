"""Round 60b attempt 43: asset-manager positioning extremes (frozen per
reference/goal_ledger.md). 4 selectable cells + mid-band diagnostics,
single OOS evaluation. Outputs results/r60b_am.json."""
...
for idx, names in SPLICE.items():
    ...
    net = (s.am_l - s.am_s) / s.oi.replace(0, np.nan)
...
if winner is None:
    ...
    json.dump(dict(grid=[{x: v for x, v in r.items() if not x.startswith("_")} for r in rows],
              ...
              open("results/r60b_am.json", "w"), indent=1, default=float)
    raise SystemExit
