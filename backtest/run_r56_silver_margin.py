"""Round 56 attempt 38: silver margin-cascade continuation (frozen per
reference/goal_ledger.md). 4 selectable cells + sub-threshold
diagnostics, single OOS evaluation. Outputs results/r56_silver_margin.json."""
...
px = pd.Series(sv.close.values, index=ts).sort_index().tz_convert("America/New_York")
px.index = px.index.tz_localize(None)

rows_d = []
for d0, g in px.groupby(px.index.date):
    base = pd.Timestamp(str(d0))
    i = g.index.searchsorted(base + pd.Timedelta(hours=9, minutes=30))
    e930 = g.iloc[i] if i < len(g) and (g.index[i] - base).total_seconds() <= 11 * 3600 else np.nan
    c16b = g[g.index <= base + pd.Timedelta(hours=16)]
    rows_d.append(dict(day=d0, e930=e930, c16=(c16b.iloc[-1] if len(c16b) else np.nan),
                       hi=g.max(), lo=g.min()))
...
print("\n=== IS grid (SILVER, ATR-normalized; sub-threshold cells diagnostic) ===")
...
if winner is None:
    ...
    json.dump(dict(grid=[{x: v for x, v in r.items() if not x.startswith("_")} for r in rows],
              ...
              open("results/r56_silver_margin.json", "w"), indent=1, default=float)
    raise SystemExit
