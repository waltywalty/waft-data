"""Round 50 attempt 31: implied-correlation spike reversal (frozen per
reference/goal_ledger.md). 4 selectable cells + low-correlation diagnostics,
single OOS evaluation. Outputs results/r50_corr.json."""
...
cr = pd.read_csv("data/COR1M_history_cboe.csv")
corr = pd.Series(cr.CLOSE.values, index=pd.to_datetime(cr.DATE).dt.date)
pct = corr.rolling(252).rank(pct=True)
print(f"COR1M: {len(corr)} days {corr.index[0]}..{corr.index[-1]}")
...
CELLS = [("high", thr, hold) for thr in (0.8, 0.9) for hold in (1, 3)] + \
        [("low_diag", 0.2, hold) for hold in (1, 3)]
...
print("\n=== IS grid (indices pooled, ATR-normalized; low-corr and gold diagnostic) ===")
...
if winner is None:
    print("\nNo high-corr cell passes the IS floor; family fails at IS, OOS not opened.")
    json.dump(dict(split=split, grid=[{x: v for x, v in r.items() if not x.startswith("_")} for r in rows],
                   winner=None, gate_pass=False),
              open("results/r50_corr.json", "w"), indent=1, default=float)
    raise SystemExit

# self-refutation check (attempt-25 lesson): spike premium must beat the low-corr drift
lo = [r for r in rows if r["kind"] == "low_diag" and r["hold"] == winner["hold"]]
if lo and (lo[0]["IS"].get("avg_R") or -9) >= (winner["IS"].get("avg_R") or -9):
    print(f"\nSELECTED {winner['thr']}/{winner['hold']}d but low-corr diagnostic matches it "
